...
def validate_data(age, investment_amount,intent_request):
    ...
    #Validate the age is greater than zero and less than 65
    if age is not None:
        age=parse_int(age)
        if age <=0 or age >65:
    ...
    if investment_amount is not None:
        ...
        if investment_amount < 5000:
            return build_validation_result(
                False,
                "investmentAmount",
                "The amount to invest should be greater or equal than 5000, "
...
def get_recommendation(risk_level,investment_amount):
    """
    Returns investment recommendation based on the selected risk level.
    """
    
    recommendation = ""
    investment_amount= parse_int(investment_amount)
    if risk_level =='None' or risk_level=='none':
        agg=round(1*investment_amount,2)
        spy=investment_amount-agg
        recommendation = recommendation+f"100% bonds (AGG), 0% equities (SPY).  In dollars, ${agg} in bonds(AGG), ${spy} in equities (SPY) ."

    elif risk_level =='Low' or risk_level=='low':
        agg=round(0.6*investment_amount,2)
        spy=investment_amount-agg
        recommendation = recommendation+f"60% bonds (AGG), 40% equities (SPY).  In dollars, ${agg} in bonds(AGG), ${spy} in equities (SPY) ."
        
    elif risk_level =='Medium' or risk_level=='medium':
        agg=round(0.4*investment_amount,2)
        spy=investment_amount-agg
        recommendation = recommendation+f"40% bonds (AGG), 60% equities (SPY).  In dollars, ${agg} in bonds(AGG), ${spy} in equities (SPY) ."
    elif risk_level =='High' or risk_level=='high':
        agg=round(0.2*investment_amount,2)
        spy=investment_amount-agg
        recommendation = recommendation+f"20% bonds (AGG), 80% equities (SPY).  In dollars, ${agg} in bonds(AGG), ${spy} in equities (SPY) ."
    else:
        recommendation = "Error.  Please try again."
...
#Gets the invocation source, for Lex dialogs "DialogCodeHook" is expected.
    ...
    return close(
        ...
        "Fulfilled",
        {
            ...
            we recommend your investment portfolio to include {}.
            """.format(
                get_recommendation(risk_level,investment_amount)
                ),
        }
    )
